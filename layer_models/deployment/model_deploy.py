"""Deploy Layers models across multiple clones and replicas.
...
Usage:
  ...
  with tf.device(config.inputs_device()):
    ...
    inputs_queue = framework.data.prefetch_queue((images, labels))
  ...
  def model_fn(inputs_queue):
    ...
    tf.losses.log_loss(predictions, labels)

  model_dp = model_deploy.deploy(config, model_fn, [inputs_queue],
                                 optimizer=optimizer)

  # Run training.
  tf.contrib.training.train(model_dp.train_op, my_log_dir,
...
'''
  ...
  framework variable creation functions: model and global variables will be created
...
'''
  ...
  args = args or []
  kwargs = kwargs or {}
  with arg_scope([framework.model_variable, framework.variable],
...
def deploy(config,
  ...
  """Deploys a Layers-constructed model across multiple clones.
...
class DeploymentConfig(object):
  ...
  def variables_device(self):
    ...
    class _PSDeviceChooser(object):
      """ device chooser for variables when using PS."""
