...
tf.flags.DEFINE_integer('max_steps', 2000,
                       'Maximum train steps.')
...
tf.flags.DEFINE_string('model_name', 'cifar10-cnn-model',
                       'Model Name.')
tf.flags.DEFINE_boolean('use_checkpoint', True,
...
class cifar10(object):
    ...
    def  generate_input_fn (self,file_names, mode=tf.estimator.ModeKeys.EVAL, batch_size=1):
        def _input_fn():
            ...
            dataset = dataset.map(lambda image, label: (self.preprocess_image(image, is_training), label))
    ...
    def model_fn(self,features, labels, mode, params):
        ...
        feature_columns = list(self.get_feature_columns().values())
    ...
    def serving_input_fn(self):
        ...
        features = {'images': tf.map_fn(self.preprocess_image, receiver_tensor['images'])}
...
estimator = tf.estimator.Estimator(model_fn=tf.contrib.estimator.replicate_model_fn(cifar.model_fn), config=run_config)

# There is another Exporter named FinalExporter
exporter = tf.estimator.LatestExporter(
  name='Servo',
  serving_input_receiver_fn=cifar.serving_input_fn,
  assets_extra=None,
  as_text=False,
  exports_to_keep=5)
...
train_spec = tf.estimator.TrainSpec(
  input_fn=cifar.generate_input_fn(file_names=train_data_files,
                             mode=tf.estimator.ModeKeys.TRAIN,
                             batch_size=FLAGS.batch_size),
  max_steps=FLAGS.max_steps)

eval_spec = tf.estimator.EvalSpec(
  input_fn=cifar.generate_input_fn(file_names=valid_data_files,
...
test_input_fn = cifar.generate_input_fn(file_names=test_data_files,
                                  mode=tf.estimator.ModeKeys.EVAL,
                                  batch_size=1000)
estimator = tf.estimator.Estimator(model_fn=cifar.model_fn, config=run_config)
print(estimator.evaluate(input_fn=test_input_fn, steps=2))
